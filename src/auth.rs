...
impl ResolverAuth {
    ...
    fn audience_allowed(&self, response: &IntrospectionResponse) -> bool {
        if self.audience.is_empty() {
            return true;
        }

        match response.aud.as_ref() {
            Some(AudienceValue::One(audience)) => audience == &self.audience,
            Some(AudienceValue::Many(audiences)) => {
    ...
    pub async fn validate_authorization_header(
        ...
        if !self.audience_allowed(&introspection) {
            return Err(ResolverAuthError::Unauthorized {
                error: "invalid_token",
        ...
        Err(ResolverAuthError::Forbidden {
...
mod tests {
    use super::{
        ResolverAuth, header_quote, normalize_url_identifier, oauth_protected_resource_metadata,
    };
